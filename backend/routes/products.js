...
function getSortClause(sort) {
  ...
  const stockPin = `CASE WHEN GREATEST(0, p.stock - COALESCE(SUM(sr.quantity), 0)) = 0 THEN 1 ELSE 0 END ASC`
  switch (key) {
    case 'price_asc':
    ...
    case 'popularity':
      return `ORDER BY ${stockPin}, COALESCE(SUM(oi.quantity), 0) DESC`
    default:
